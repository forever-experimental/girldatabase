...
const {DYNAMODB_TABLE, CORS_PASS} = env;
...
const s3Client = new S3Client({
    endpoint: env.CLOUDFLARE_ENDPOINT, region: 'auto', credentials: {
        accessKeyId: env.CLOUDFLARE_ACCESS_KEY_ID, secretAccessKey: env.CLOUDFLARE_SECRET_ACCESS_KEY,
...
export default async function fetch(request)
{
    ...
    if (!rateLimitData) {
        rateLimitData = {
            count: 1, expiry: Date.now() + RATE_LIMIT_TIME_WINDOW * 1000,
        };
    }
    else {
        rateLimitData.count++;
    ...
    try {
        ...
        await s3Client.send(new PutObjectCommand({
            Bucket: bucketName, Key: hashName, Body: processedBuffer, Metadata: metadata,
        }));

        const item = {
            TableName: DYNAMODB_TABLE, Item: {
                dir: {S: dir},
